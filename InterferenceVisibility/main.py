...
def main() -> None:
    ...
    interference = []
    for temperature in temperature_it:
        TEC.set_temperature(float(temperature))
        TEC.wait_until_stable(SETTINGS["TEC"]["tolerance"])
        mean_amp, std_amp = interference_voltage_estimation(OSC)
        interference.append((float(temperature), float(mean_amp), float(std_amp)))
    ...
    dir=os.path.join("measurements",file_name)
    os.mkdir(dir)
    with open(os.path.join(dir,"data.json"), mode="w") as f:
        json.dump(data_to_save, f, indent=2)

    # Prepare plotting, skip any NaN measurements
    temps = [t for (t, m, s) in interference if not np.isnan(m)]
    means = [m for (t, m, s) in interference if not np.isnan(m)]
    std = [s for (t,m,s) in interference if not np.isnan(m)]
    if len(temps) == 0:
    ...
    plt.legend()
    plt.savefig(os.path.join(dir,"fig.png"))
    plt.grid(True)
    plt.show()
